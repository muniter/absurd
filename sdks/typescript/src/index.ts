...
export interface PendingTaskInfo extends BaseTaskInfo {
  status: "pending";
}

export interface RunningTaskInfo extends BaseTaskInfo {
  status: "running";
  firstStartedAt: Date;
  lastAttemptRunId: string;
}

export interface SleepingTaskInfo extends BaseTaskInfo {
  status: "sleeping";
  firstStartedAt: Date;
  lastAttemptRunId: string;
}

export interface CompletedTaskInfo<T = JsonValue> extends BaseTaskInfo {
  status: "completed";
  result: T;
  completedAt: Date;
  firstStartedAt: Date;
...
export interface FailedTaskInfo extends BaseTaskInfo {
  status: "failed";
  error: JsonValue;
  failedAt: Date | null;
}

export interface CancelledTaskInfo extends BaseTaskInfo {
  status: "cancelled";
  cancelledAt: Date;
}

export type TaskInfo<T = JsonValue> =
  | PendingTaskInfo
  | RunningTaskInfo
  | SleepingTaskInfo
  | CompletedTaskInfo<T>
...
export interface PendingRunInfo extends BaseRunInfo {
  status: "pending";
  availableAt: Date;
}

export interface RunningRunInfo extends BaseRunInfo {
  status: "running";
  startedAt: Date | null;
}

export interface SleepingRunInfo extends BaseRunInfo {
  status: "sleeping";
  wakeAt: Date;
}

export interface CompletedRunInfo<T = JsonValue> extends BaseRunInfo {
  status: "completed";
...
export interface FailedRunInfo extends BaseRunInfo {
  status: "failed";
  failedAt: Date | null;
  error: JsonValue;
  startedAt?: Date | null;
...
export interface CancelledRunInfo extends BaseRunInfo {
  status: "cancelled";
  startedAt?: Date | null;
}

export type RunInfo<T = JsonValue> =
  | PendingRunInfo
  | RunningRunInfo
  | SleepingRunInfo
  | CompletedRunInfo<T>
...
export class Absurd {
  ...
  /**
   * Gets the current state of a task, including its final result if completed.
   */
  async getTask<T = JsonValue>(taskID: string, queueName?: string): Promise<TaskInfo<T> | null> {
    const queue = queueName ?? this.queueName;
    const { rows } = await this.pool.query<TaskQueryRow>(
      `
      SELECT
        t.task_id,
        t.state,
    ...
    switch (r.state) {
      case "completed":
        return {
          taskID: r.task_id,
          status: r.state,
          result: r.completed_payload as T,
          attempts: r.attempts,
          firstStartedAt: r.first_started_at!,
          completedAt: r.completed_at!,
          lastAttemptRunId: r.last_attempt_run!,
        };
      case "failed":
        return {
          taskID: r.task_id,
      ...
      default: {
        const _typeExhaustiveCheck: never = r.state;
        throw new Error(`Unknown task state: ${_typeExhaustiveCheck}`);
      }
    }
  }

  /**
   * Gets the current state of a specific run (execution attempt).
   */
  async getRun<T = JsonValue>(runID: string, queueName?: string): Promise<RunInfo<T> | null> {
    const queue = queueName ?? this.queueName;
    const { rows } = await this.pool.query<RunQueryRow>(
      `
      SELECT
        run_id,
        task_id,
